...
function PRng:new(seed)
    prng = {}
    setmetatable(prng, self)
    self.__index = self
    prng:set_seed(seed or 0)
    return prng
end

function PRng:set_seed(seed_53)
    -- set 53-bit integer as current seed (seed is initially set to 0 when program starts)
    self.state_45 = seed_53 % 35184372088832
    self.state_8 = floor(seed_53 / 35184372088832) % 255 + 2
end

function PRng:get_seed()
    -- returns current seed as single 53-bit integer
    return (self.state_8 - 2) * 35184372088832 + self.state_45
end

function PRng:get_random_32()
